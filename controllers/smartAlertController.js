...
export async function generateSmartAlert(req, res) {
  try {
    ...
    // Get Bangla translations
    const cropBn = CROP_TRANSLATIONS[cropType] || cropType;
    const storageBn = STORAGE_TRANSLATIONS[storageType] || storageType || 'গুদাম';
    ...
    const chatCompletion = await client.chatCompletion({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert agricultural advisor for Bangladeshi farmers. 
You MUST respond ONLY in Bangla (Bengali) language using Bengali script.
Your advice should be:
1. Specific and actionable
2. Based on the exact crop, weather, and risk data provided
3. Brief (2-3 sentences maximum)
4. Include immediate actions the farmer can take

IMPORTANT: Never use English. Always use Bangla script like: আগামীকাল, বৃষ্টি, আর্দ্রতা, ফ্যান চালু করুন, etc.`
        },
        {
          role: "user",
          content: prompt
    ...
    const alertMessage = chatCompletion.choices[0]?.message?.content || '';

    // Determine if SMS should be simulated
    ...
    // Fallback to template-based alert if LLM fails
...
function buildBanglaPrompt(data) {
  const {
    cropType, cropBn, storageType, storageBn,
    division, district, riskLevel, riskBn,
    etcl, temperature, humidity, rainProb, moisture
  } = data;
...
function generateFallbackAlert(data) {
  const {
    cropType, storageType, riskLevel,
    temperature, humidity, rainProb, etcl
  } = data;
